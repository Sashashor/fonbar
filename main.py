...
async def miniapp_order_handler(request: web.Request) -> web.Response:
    ...
    try:
        if order_type == "stars":
            cursor.execute("UPDATE total_stars SET total = total + ? WHERE id = 1", (int(amount),))
        ...
        logging.warning("Failed to update miniapp global counters: %s", error)
...
async def confirm_delete_callback(callback: types.CallbackQuery):
    ...
    if history_id is not None:
        try:
            set_miniapp_purchase_history_status(history_id, "success")
        except Exception as error:
            logging.warning("Failed to update miniapp history status to success. id=%s error=%s", history_id, error)
    try:
        await callback.message.delete()
