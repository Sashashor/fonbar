...
def build_operation_id(source: str, record_id: int, created_at: str = "") -> str:
    ...
    if not day_label:
        day_label = datetime.utcnow().strftime("%d.%m.%y")
...
async def miniapp_order_handler(request: web.Request) -> web.Response:
    ...
    if not operation_created_at:
        operation_created_at = datetime.utcnow().replace(microsecond=0).isoformat()
    ...
    return miniapp_json_response(
        {
            ...
            "operation": {
                "id": build_operation_id("miniapp", history_id, operation_created_at),
