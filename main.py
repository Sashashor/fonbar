...
SPEND_SERVICE_BASE_URL = str(os.getenv("SPEND_SERVICE_BASE_URL") or "").strip().rstrip("/")
SPEND_SERVICE_TOKEN = str(os.getenv("SPEND_SERVICE_TOKEN") or "").strip()
try:
...
def _spend_service_estimate_path_candidates() -> tuple:
    base = SPEND_SERVICE_BASE_URL.lower()
...
async def _spend_service_estimate_ton_price(item_type: int, amount: int, fee_percent: float) -> float:
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        for path_index, path in enumerate(path_candidates):
            url = f"{SPEND_SERVICE_BASE_URL}{path}"
...
async def _admin_get_spend_market_rates_payload() -> dict:
    ...
    if not _spend_service_is_configured():
        base_payload["error"] = "SPEND API не настроен (SPEND_SERVICE_BASE_URL/SPEND_SERVICE_TOKEN)"
        return base_payload
