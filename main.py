...
async def miniapp_order_handler(request: web.Request) -> web.Response:
    ...
    if payment_method_raw in {"ton", "ton_wallet", "tonwallet"}:
        ...
        return miniapp_json_response(
            {
                "ok": False,
                "error": "Метод USDT TON пока недоступен",
                "errorCode": "MAPP-PAY-USDT-001",
            },
            status=503,
        )
    else:
    ...
    try:
        history_id = add_miniapp_purchase_history(
            ...
            status="pending",
            counters_applied=0,
            source="api_ton" if payment_method == "ton_wallet" else "api",
        )
    except Exception as error:
    ...
    if payment_method == "ton_wallet":
        ...
        merchant_wallet = normalize_ton_wallet_address(TON_PAYMENT_WALLET)
        if not merchant_wallet or not is_valid_ton_wallet_address(merchant_wallet):
            set_miniapp_purchase_history_error(
                history_id,
                "TON_PAYMENT_WALLET не настроен или некорректен.",
                status="error",
            )
            return miniapp_json_response(
                {"ok": False, "error": "TON-кошелёк магазина не настроен", "errorCode": "MAPP-TON-CFG-001"},
                status=503,
            )
...
async def confirm_delete_callback(callback: types.CallbackQuery):
    ...
    if history_id is not None:
        try:
            ...
            if finalize_result.get("ok"):
                history_details = get_miniapp_purchase_history_details(history_id)
                if str(history_details.get("source") or "").strip().lower() == "api_ton":
                    ton_bill_id = f"ton-{int(history_id)}"
                    save_payment(
                        int(history_details.get("userId") or 0),
                        ton_bill_id,
                        round(float(history_details.get("priceRub") or 0.0), 2),
                        "paid",
                        f"miniapp payment paid, history_id={int(history_id)}, provider=ton_wallet, confirmed_by_admin={int(getattr(callback.from_user, 'id', 0) or 0)}",
                    )
                await miniapp_broadcast_event(
                    "purchase_finalized",
                    {
