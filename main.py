...
async def create_freekassa_order(
    *,
    ...
    if FK_SUCCESS_URL:
        request_payload["successUrl"] = FK_SUCCESS_URL
    if FK_FAIL_URL:
        request_payload["failUrl"] = FK_FAIL_URL

    request_signature = build_freekassa_api_signature(request_payload)
    headers = {
        "Content-Type": "application/json",
        "Accept": "application/json",
        "MerchantId": str(FK_MERCHANT_ID),
