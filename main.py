...
REVIEWS_GROUP_CHAT_ID = -1002506761866
...
def init_db():
    ...
    history_status_column_added = False
    try:
        cursor.execute("ALTER TABLE miniapp_purchase_history ADD COLUMN status TEXT DEFAULT 'pending'")
        history_status_column_added = True
    except sqlite3.OperationalError:
        pass
    history_counters_column_added = False
    try:
        cursor.execute("ALTER TABLE miniapp_purchase_history ADD COLUMN counters_applied INTEGER DEFAULT 0")
        history_counters_column_added = True
    except sqlite3.OperationalError:
        pass
...
def upsert_miniapp_review(
    *,
    ...
    created_at_value = str(created_at or "").strip() or datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")
    cursor.execute(
        """
...
def get_miniapp_reviews_payload(limit: int = 30) -> list:
...
def finalize_miniapp_purchase_history(history_id: int) -> dict:
    ...
    try:
        ...
        cursor.execute(
            """
            SELECT user_id, item_type, amount, LOWER(TRIM(COALESCE(status, ''))), COALESCE(counters_applied, 0)
        ...
        user_id, item_type, raw_amount, current_status, counters_applied = row
...
def get_miniapp_purchase_history_payload(user_id: int, limit: int = 20) -> list:
    ...
    cursor.execute(
        '''
        SELECT item_type, amount, price_usd, price_rub, created_at, promo_code, promo_error, status
        FROM miniapp_purchase_history
        WHERE user_id=?
        ORDER BY id DESC
        LIMIT ?
    ...
    for item_type, amount, price_usd, price_rub, created_at, promo_code, promo_error, raw_status in miniapp_rows:
    ...
    for item_type, amount, cost, created_at in legacy_rows:
        price_rub = float(cost or 0.0)
        price_usd = round(price_rub / MINIAPP_USD_RUB_RATE, 2) if MINIAPP_USD_RUB_RATE > 0 else 0.0
        combined_rows.append((item_type, amount, price_usd, price_rub, created_at, "", "", "success"))

    combined_rows.sort(key=lambda row: str(row[4] or ""), reverse=True)

    payload = []
    for item_type, amount, price_usd, price_rub, created_at, promo_code, promo_error, raw_status in combined_rows[:limit]:
        normalized_type = str(item_type or "").strip().lower()
...
@dp.message(lambda m: bool(getattr(m, "chat", None)) and int(getattr(m.chat, "id", 0) or 0) == REVIEWS_GROUP_CHAT_ID)
async def ingest_review_group_message(message: types.Message):
    ...
    avatar_file_id = await get_user_avatar_file_id(int(from_user.id or 0))
    created_at = (message.date or datetime.utcnow()).strftime("%Y-%m-%d %H:%M:%S")

    try:
        upsert_miniapp_review(
            chat_id=int(message.chat.id),
            message_id=int(message.message_id or 0),
            reviewer_user_id=int(from_user.id or 0),
            reviewer_username=(from_user.username or "").strip(),
            reviewer_first_name=(from_user.first_name or "").strip(),
            reviewer_last_name=(from_user.last_name or "").strip(),
        ...
        await miniapp_broadcast_event(
            "reviews_updated",
            {
                "chatId": int(REVIEWS_GROUP_CHAT_ID),
