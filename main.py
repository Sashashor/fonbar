...
def build_history_price_label(price_usd: float, price_rub: float) -> str:
    ...
    rub_text = f"{int(rub_value)}₽" if rub_value.is_integer() else f"{rub_value:.2f}₽"
...
async def miniapp_admin_action_handler(request: web.Request) -> web.Response:
    ...
    try:
        ...
        if action == "stats_users_list":
            ...
            if query:
                ...
                if query.isdigit():
                    ...
                    where_params = [int(query), like_value]
                else:
        ...
        if action == "promo_list":
            ...
            cursor.execute(
                """
                SELECT code, discount_percent, min_stars, expires_at, max_uses, uses_count, condition, target, max_uses_per_user, max_free_stars
                FROM promo_codes
                WHERE expires_at >= ? AND uses_count < max_uses
                ...
                promos.append(
                    {
                        ...
                        "expiresAt": str(row[3] or ""),
                        "maxUses": int(row[4] or 0),
                        "usesCount": int(row[5] or 0),
        ...
        if action == "promo_create":
            ...
            discount = _admin_parse_positive_int(action_payload.get("discount"), field_name="discount")
            if discount > 100:
                raise ValueError("discount не может быть больше 100")
            ...
            max_free_stars = _admin_parse_non_negative_int(action_payload.get("maxFreeStars"), field_name="maxFreeStars")
            target = str(action_payload.get("target") or "stars").strip().lower()
            if target not in {"stars", "premium", "all"}:
                raise ValueError("target должен быть stars, premium или all")

            cursor.execute(
                """
                INSERT OR REPLACE INTO promo_codes
                (code, discount_percent, min_stars, expires_at, max_uses, uses_count, condition, max_free_stars, target, max_uses_per_user)
                VALUES (?, ?, ?, ?, ?, 0, ?, ?, ?, ?)
                """,
                (
                    code,
            ...
            conn.commit()
            return miniapp_json_response({"ok": True, "data": {"code": code}})
