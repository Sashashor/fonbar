...
def _ensure_miniapp_purchase_history_schema() -> tuple:
...
def add_miniapp_purchase_history(
    *,
    ...
    try:
        ...
        if "has no column named" in error_text or "no such column" in error_text:
            _ensure_miniapp_purchase_history_schema()
            cursor.execute(insert_sql, insert_params)
        else:
            raise
...
async def miniapp_order_handler(request: web.Request) -> web.Response:
    ...
    try:
        ...
        return miniapp_json_response(
            {"ok": False, "error": "Ошибка базы данных при сохранении заявки. Попробуйте снова."},
            status=500,
        )
