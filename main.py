...
MINIAPP_ALLOW_SENDDATA_FALLBACK = env_flag("MINIAPP_ALLOW_SENDDATA_FALLBACK", default=False)
APP_TIMEZONE_NAME = str(os.getenv("APP_TIMEZONE") or "Europe/Simferopol").strip() or "Europe/Simferopol"
TON_PAYMENT_ENABLED = env_flag("TON_PAYMENT_ENABLED", default=True)
TON_PAYMENT_WALLET = str(
    os.getenv("TON_PAYMENT_WALLET")
    or os.getenv("TON_MERCHANT_WALLET")
    or ""
).strip()
TON_PAYMENT_MEMO_PREFIX = str(os.getenv("TON_PAYMENT_MEMO_PREFIX") or "SLX").strip().upper() or "SLX"
TON_USDT_PAYMENT_ENABLED = env_flag("TON_USDT_PAYMENT_ENABLED", default=True)
TON_USDT_PAYMENT_WALLET = str(
    os.getenv("TON_USDT_PAYMENT_WALLET")
    or os.getenv("TON_USDT_WALLET")
    or TON_PAYMENT_WALLET
    or ""
).strip()
TON_USDT_JETTON_MASTER = str(
    os.getenv("TON_USDT_JETTON_MASTER")
    or os.getenv("TON_USDT_JETTON_CONTRACT")
    or ""
).strip()
TON_USDT_MEMO_PREFIX = (
    str(os.getenv("TON_USDT_MEMO_PREFIX") or f"{TON_PAYMENT_MEMO_PREFIX}U")
    .strip()
    .upper()
    or "SLXU"
)
try:
    TON_USDT_DECIMALS = int(os.getenv("TON_USDT_DECIMALS", "6"))
except (TypeError, ValueError):
    TON_USDT_DECIMALS = 6
TON_USDT_DECIMALS = max(0, min(TON_USDT_DECIMALS, 18))
...
try:
    MINIAPP_INITDATA_FUTURE_SKEW_SECONDS = int(os.getenv("MINIAPP_INITDATA_FUTURE_SKEW_SECONDS", "120"))
except (TypeError, ValueError):
    MINIAPP_INITDATA_FUTURE_SKEW_SECONDS = 120
MINIAPP_INITDATA_MAX_AGE_SECONDS = max(0, MINIAPP_INITDATA_MAX_AGE_SECONDS)
MINIAPP_INITDATA_FUTURE_SKEW_SECONDS = max(0, MINIAPP_INITDATA_FUTURE_SKEW_SECONDS)
...
def init_db():
    ...
    cursor.execute(
        '''
        CREATE TABLE IF NOT EXISTS miniapp_user_wallets (
            user_id INTEGER PRIMARY KEY,
...
def _miniapp_default_error_code(status: int) -> str:
    status_code = int(status or 500)
    if status_code == 400:
        return "MAPP-REQ-400"
    if status_code == 401:
        return "MAPP-AUTH-401"
    if status_code == 403:
        return "MAPP-AUTH-403"
    if status_code == 404:
        return "MAPP-REQ-404"
    if status_code == 409:
        return "MAPP-REQ-409"
...
def miniapp_json_response(payload: dict, status: int = 200) -> web.Response:
    safe_payload = dict(payload or {}) if isinstance(payload, dict) else {"ok": False, "error": "Invalid response payload"}
    if safe_payload.get("ok") is False:
        error_text = str(safe_payload.get("error") or "").strip()
        safe_payload["error"] = error_text or "Request failed"
        safe_payload.setdefault("errorCode", _miniapp_default_error_code(status))
        safe_payload.setdefault("stage", "api")
        safe_payload.setdefault("statusCode", int(status))
    return web.json_response(safe_payload, status=status, headers=CORS_HEADERS)
...
def miniapp_error_response(
    ...
    *,
    ...
    payload = {
        "ok": False,
        "error": str(error_text or "Request failed"),
        "stage": str(stage or "api"),
    }
    if error_code:
        payload["errorCode"] = str(error_code).strip()
    if isinstance(extra, dict) and extra:
        payload.update(extra)
    return miniapp_json_response(payload, status=status)
...
def normalize_ton_wallet_address(raw_value: str) -> str:
    return str(raw_value or "").strip()


def is_valid_ton_wallet_address(raw_value: str) -> bool:
    address = normalize_ton_wallet_address(raw_value)
    if not address:
        return False
    if re.fullmatch(r"(?:-1|0):[0-9a-fA-F]{64}", address):
        return True
    if re.fullmatch(r"[A-Za-z0-9_-]{48,128}", address):
        return True
    return False


def build_ton_transfer_links(wallet_address: str, amount_nano: int, memo: str) -> dict:
    safe_wallet = normalize_ton_wallet_address(wallet_address)
    safe_nano = max(1, int(amount_nano or 0))
    safe_memo = str(memo or "").strip()
    encoded_memo = quote(safe_memo, safe="") if safe_memo else ""
    query = f"amount={safe_nano}"
    if encoded_memo:
        query = f"{query}&text={encoded_memo}"
    return {
        "deepLink": f"ton://transfer/{safe_wallet}?{query}",
        "tonkeeperLink": f"https://app.tonkeeper.com/transfer/{safe_wallet}?{query}",
    }


def build_ton_usdt_transfer_links(
    wallet_address: str,
    amount_units: int,
    memo: str,
    jetton_master: str,
    ...
    safe_wallet = normalize_ton_wallet_address(wallet_address)
    safe_units = max(1, int(amount_units or 0))
    safe_memo = str(memo or "").strip()
    safe_jetton = normalize_ton_wallet_address(jetton_master)

    query_parts = [
        f"amount={safe_units}",
        f"jetton={quote(safe_jetton, safe='')}",
    ]
    if safe_memo:
        query_parts.append(f"text={quote(safe_memo, safe='')}")
...
async def resolve_checkout_rub_per_ton() -> float:
    try:
        market_payload = await _admin_get_spend_market_rates_payload()
    except Exception:
        market_payload = {}

    if isinstance(market_payload, dict):
        try:
            rub_per_ton_spend = float(market_payload.get("rubPerTon") or 0.0)
        except (TypeError, ValueError):
            rub_per_ton_spend = 0.0
        if rub_per_ton_spend > 0:
            return rub_per_ton_spend

    try:
        rub_per_ton_cg = float(await get_rub_to_ton_rate())
    except Exception as error:
        raise RuntimeError(f"Не удалось получить курс TON/RUB: {error}")

    if rub_per_ton_cg <= 0:
        raise RuntimeError("Не удалось получить корректный курс TON/RUB")
    return rub_per_ton_cg
...
def miniapp_extract_init_data(request: web.Request, payload: dict = None) -> str:
    init_data = ""
    if isinstance(payload, dict):
...
def parse_telegram_init_data(init_data: str) -> dict:
    ...
    auth_date_raw = str(parsed_items.get("auth_date") or "").strip()
    if not auth_date_raw:
        raise ValueError("Missing Telegram auth_date")
    try:
        auth_date = int(auth_date_raw)
    except (TypeError, ValueError) as error:
        raise ValueError("Invalid Telegram auth_date") from error

    now_ts = int(app_now().timestamp())
    if auth_date > now_ts + int(MINIAPP_INITDATA_FUTURE_SKEW_SECONDS):
        raise ValueError("Telegram init data auth_date is invalid")
    max_age_seconds = int(MINIAPP_INITDATA_MAX_AGE_SECONDS)
...
async def miniapp_events_handler(request: web.Request) -> web.StreamResponse:
    ...
    init_data = miniapp_extract_init_data(request)
    if not init_data:
        return miniapp_error_response("Missing Telegram init data", status=400)

    try:
        telegram_user = parse_telegram_init_data(init_data)
    except ValueError as error:
        return miniapp_error_response(str(error), status=401)

    user_id = int(telegram_user.get("id") or 0)
    if user_id <= 0:
        return miniapp_error_response("Invalid Telegram user id", status=401)
...
def get_miniapp_user_ton_wallet(user_id: int) -> str:
    safe_user_id = int(user_id or 0)
    if safe_user_id <= 0:
        return ""
    cursor.execute(
        "SELECT ton_wallet FROM miniapp_user_wallets WHERE user_id = ? LIMIT 1",
        (safe_user_id,),
    )
    row = cursor.fetchone()
    return normalize_ton_wallet_address((row or [""])[0] or "")


def set_miniapp_user_ton_wallet(user_id: int, wallet_address: str) -> str:
    safe_user_id = int(user_id or 0)
    if safe_user_id <= 0:
        raise ValueError("Некорректный user_id")
    normalized_wallet = normalize_ton_wallet_address(wallet_address)
    if normalized_wallet and not is_valid_ton_wallet_address(normalized_wallet):
        raise ValueError("Некорректный TON-кошелёк")
    cursor.execute(
        """
        INSERT INTO miniapp_user_wallets (user_id, ton_wallet, updated_at)
        VALUES (?, ?, ?)
        ON CONFLICT(user_id) DO UPDATE SET
            ton_wallet = excluded.ton_wallet,
            updated_at = excluded.updated_at
        """,
        (safe_user_id, normalized_wallet, app_now_iso()),
    )
    conn.commit()
...
def get_miniapp_purchase_history_details(history_id: int) -> dict:
    ...
    return {
        ...
        "source": str(row[15] or "").strip().lower(),
        "countersApplied": int(row[16] or 0),
    }
...
async def miniapp_support_photo_handler(request: web.Request) -> web.Response:
    ...
    init_data = miniapp_extract_init_data(request)
    if not init_data:
        return web.Response(status=401, headers=CORS_HEADERS)
    try:
        telegram_user = parse_telegram_init_data(init_data)
    except ValueError:
        return web.Response(status=401, headers=CORS_HEADERS)

    requester_user_id = int(telegram_user.get("id") or 0)
    if requester_user_id <= 0:
        return web.Response(status=401, headers=CORS_HEADERS)

    cursor.execute(
        """
        SELECT m.photo_blob, m.photo_mime, c.user_id
        FROM miniapp_support_messages m
        JOIN miniapp_support_chats c ON c.id = m.chat_id
        WHERE m.id = ?
        LIMIT 1
        """,
        (message_id,),
    ...
    has_access = requester_user_id in ADMIN_IDS or requester_user_id == owner_user_id
    if not has_access:
        return web.Response(status=403, headers=CORS_HEADERS)
...
async def miniapp_support_action_handler(request: web.Request) -> web.Response:
    ...
    init_data = miniapp_extract_init_data(request, payload)
    if not init_data:
        return miniapp_error_response("Missing Telegram init data", status=400)
...
async def miniapp_profile_handler(request: web.Request) -> web.Response:
    ...
    if action == "bind_ton_wallet":
        raw_wallet = str(payload.get("wallet") or payload.get("tonWallet") or "").strip()
        normalized_wallet = normalize_ton_wallet_address(raw_wallet)
        if not normalized_wallet:
            return miniapp_json_response({"ok": False, "error": "Введите TON-кошелёк"}, status=400)
        if not is_valid_ton_wallet_address(normalized_wallet):
            return miniapp_json_response({"ok": False, "error": "Некорректный TON-кошелёк"}, status=400)
        set_miniapp_user_ton_wallet(user_id, normalized_wallet)

    if action == "unbind_ton_wallet":
        set_miniapp_user_ton_wallet(user_id, "")
    ...
    ton_wallet = get_miniapp_user_ton_wallet(user_id)
...
async def miniapp_admin_open_handler(request: web.Request) -> web.Response:
    ...
    if user_id not in ADMIN_IDS:
        return miniapp_error_response("Access denied", status=403)
...
async def miniapp_order_handler(request: web.Request) -> web.Response:
    ...
    payment_method_raw = str(payload.get("paymentMethod") or "").strip().lower()
    if payment_method_raw in {"ton", "ton_wallet", "tonwallet"}:
        payment_method = "ton_wallet"
    elif payment_method_raw in {"usdt_ton", "usdtton", "ton_usdt"}:
    ...
    try:
        history_id = add_miniapp_purchase_history(
            ...
            source=(
                "api_ton"
                if payment_method == "ton_wallet"
                else ("api_usdt_ton" if payment_method == "usdt_ton" else "api")
            ),
        )
    except Exception as error:
    ...
    if payment_method in {"ton_wallet", "usdt_ton"}:
        is_usdt_ton = payment_method == "usdt_ton"
        if is_usdt_ton:
            if not TON_USDT_PAYMENT_ENABLED:
                set_miniapp_purchase_history_error(
                    history_id,
                    "USDT TON-оплата временно отключена.",
                    status="error",
                )
                return miniapp_json_response(
                    {
                        "ok": False,
                        "error": "USDT TON-оплата временно отключена.",
                        "errorCode": "MAPP-USDT-CFG-000",
                    },
                    status=503,
                )
        else:
            if not TON_PAYMENT_ENABLED:
                set_miniapp_purchase_history_error(
                    history_id,
                    "TON-оплата временно отключена.",
                    status="error",
                )
                return miniapp_json_response(
                    {"ok": False, "error": "TON-оплата временно отключена.", "errorCode": "MAPP-TON-CFG-000"},
                    status=503,
                )

        merchant_wallet = normalize_ton_wallet_address(
            TON_USDT_PAYMENT_WALLET if is_usdt_ton else TON_PAYMENT_WALLET
        )
        if not merchant_wallet or not is_valid_ton_wallet_address(merchant_wallet):
            set_miniapp_purchase_history_error(
                history_id,
                "Кошелёк магазина не настроен или некорректен.",
                status="error",
            )
            return miniapp_json_response(
                {
                    "ok": False,
                    "error": "TON-кошелёк магазина не настроен",
                    "errorCode": "MAPP-USDT-CFG-001" if is_usdt_ton else "MAPP-TON-CFG-001",
                },
                status=503,
            )

        payer_wallet_raw = str(payload.get("tonWallet") or payload.get("payerTonWallet") or "").strip()
        payer_wallet = normalize_ton_wallet_address(payer_wallet_raw) or get_miniapp_user_ton_wallet(user_id)
        if not payer_wallet:
            set_miniapp_purchase_history_error(
                history_id,
                "Не привязан TON-кошелёк пользователя.",
                status="error",
            )
            return miniapp_json_response(
                {"ok": False, "error": "Сначала привяжите TON-кошелёк", "errorCode": "MAPP-TON-WLT-001"},
                status=400,
            )
        if not is_valid_ton_wallet_address(payer_wallet):
            set_miniapp_purchase_history_error(
                history_id,
                "Некорректный TON-кошелёк пользователя.",
                status="error",
            )
            return miniapp_json_response(
                {"ok": False, "error": "Некорректный TON-кошелёк", "errorCode": "MAPP-TON-WLT-002"},
                status=400,
            )

        try:
            set_miniapp_user_ton_wallet(user_id, payer_wallet)
        except Exception:
            pass

        if is_usdt_ton:
            jetton_master = normalize_ton_wallet_address(TON_USDT_JETTON_MASTER)
            if not jetton_master or not is_valid_ton_wallet_address(jetton_master):
                set_miniapp_purchase_history_error(
                    history_id,
                    "TON_USDT_JETTON_MASTER не настроен или некорректен.",
                    status="error",
                )
                return miniapp_json_response(
                    {
                        "ok": False,
                        "error": "USDT jetton-контракт не настроен",
                        "errorCode": "MAPP-USDT-CFG-002",
                    },
                    status=503,
                )

            amount_usdt = round(float(final_usd), TON_USDT_DECIMALS)
            if amount_usdt <= 0:
                set_miniapp_purchase_history_error(
                    history_id,
                    "Некорректная сумма USDT для оплаты.",
                    status="error",
                )
                return miniapp_json_response(
                    {
                        "ok": False,
                        "error": "Некорректная сумма USDT",
                        "errorCode": "MAPP-USDT-AMT-001",
                    },
                    status=400,
                )

            usdt_multiplier = 10 ** int(TON_USDT_DECIMALS or 6)
            amount_units = max(1, int(round(amount_usdt * usdt_multiplier)))
            memo_prefix = re.sub(r"[^A-Za-z0-9_-]", "", str(TON_USDT_MEMO_PREFIX or "SLXU")) or "SLXU"
            payment_memo = f"{memo_prefix}{operation_id}"
            usdt_links = build_ton_usdt_transfer_links(
                merchant_wallet,
                amount_units,
                payment_memo,
                jetton_master,
            )
            usdt_payment_id = f"usdt-ton-{int(history_id)}"
            ...
            save_payment(
                ...
                "pending",
                (
                    f"miniapp payment pending, history_id={int(history_id)}, provider=usdt_ton, "
                    f"payer_wallet={payer_wallet}, wallet={merchant_wallet}, memo={payment_memo}, "
                    f"jetton={jetton_master}, amount_usdt={amount_usdt:.{int(TON_USDT_DECIMALS)}f}, amount_units={amount_units}"
                ),
            )
            ...
            usdt_message_text = "\n".join(
                [
                    "🧾 <b>Новая USDT TON-оплата из MINIAPP</b>",
                    f"🆔 <b>ID операции:</b> <code>{escape(operation_id)}</code>",
                    f"👤 <b>Покупатель:</b> @{escape(str(telegram_user.get('username') or f'id{user_id}'))} (ID: <code>{int(user_id)}</code>)",
                    f"🎯 <b>Аккаунт:</b> @{escape(str(target_username or ''))}",
                    f"📦 <b>Товар:</b> {escape(amount_label)}",
                    f"💰 <b>Сумма:</b> {final_rub:.2f}₽ / {final_usd:.2f}$",
                    f"💵 <b>К оплате:</b> <code>{amount_usdt:.{int(TON_USDT_DECIMALS)}f} USDT (TON)</code>",
                    f"🏦 <b>Кошелёк магазина:</b> <code>{escape(merchant_wallet)}</code>",
                    f"🧩 <b>Jetton master:</b> <code>{escape(jetton_master)}</code>",
                    f"👛 <b>Кошелёк клиента:</b> <code>{escape(payer_wallet)}</code>",
                    f"📝 <b>Комментарий:</b> <code>{escape(payment_memo)}</code>",
                    "✅ <i>После поступления USDT нажмите «Готово».</i>",
                ]
            )
            ...
            try:
                ...
                set_miniapp_purchase_history_error(
                    history_id,
                    f"Ошибка отправки USDT TON-заявки в группу: {error}",
                    status="error",
                )
                return miniapp_json_response(
                    {
                        "ok": False,
                        "error": "Не удалось отправить USDT TON-заявку в группу админов",
                        "errorCode": "MAPP-USDT-GRP-001",
                    },
                    status=502,
                )
            ...
            return miniapp_json_response(
                {
                    "ok": True,
                    "message": "USDT TON-заявка создана. Выполните перевод и дождитесь подтверждения админа.",
                    "price": {
                        "rub": round(final_rub, 2),
                        "usd": final_usd,
                    },
                    ...
                    "payment": {
                        "provider": "usdt_ton",
                        "paymentId": usdt_payment_id,
                        "url": usdt_links.get("tonkeeperLink") or "",
                        "deepLink": usdt_links.get("deepLink") or "",
                        ...
                        "decimals": int(TON_USDT_DECIMALS),
                        "method": "usdt_ton",
                        "rubPerUsdt": round(float(MINIAPP_USD_RUB_RATE), 4),
                    },
                    "operation": {
                        "id": operation_id,
        ...
        try:
            rub_per_ton = float(await resolve_checkout_rub_per_ton())
        except Exception as error:
            set_miniapp_purchase_history_error(
                history_id,
                f"Не удалось получить курс TON/RUB: {error}",
                status="error",
            )
            return miniapp_json_response(
                {
                    "ok": False,
                    "error": "Не удалось рассчитать сумму в TON. Повторите позже.",
                    "errorCode": "MAPP-TON-RATE-001",
                },
                status=502,
            )
        ...
        if rub_per_ton <= 0:
            set_miniapp_purchase_history_error(
                history_id,
                "Получен некорректный курс TON/RUB.",
                status="error",
            )
            return miniapp_json_response(
                {"ok": False, "error": "Некорректный курс TON", "errorCode": "MAPP-TON-RATE-002"},
                status=502,
            )

        amount_ton = round(float(final_rub) / rub_per_ton, 6)
        amount_nano = max(1, int(round(amount_ton * 1_000_000_000)))
        memo_prefix = re.sub(r"[^A-Za-z0-9_-]", "", str(TON_PAYMENT_MEMO_PREFIX or "SLX")) or "SLX"
        payment_memo = f"{memo_prefix}{operation_id}"
        ton_links = build_ton_transfer_links(merchant_wallet, amount_nano, payment_memo)
        ton_payment_id = f"ton-{int(history_id)}"
        ...
        save_payment(
            ...
            "pending",
            (
                f"miniapp payment pending, history_id={int(history_id)}, provider=ton_wallet, "
                f"payer_wallet={payer_wallet}, wallet={merchant_wallet}, memo={payment_memo}"
            ),
        )
        set_miniapp_purchase_history_error(history_id, "", status="pending")
        ...
        ton_message_text = "\n".join(
            [
                "🧾 <b>Новая TON-оплата из MINIAPP</b>",
                f"🆔 <b>ID операции:</b> <code>{escape(operation_id)}</code>",
                f"👤 <b>Покупатель:</b> @{escape(str(telegram_user.get('username') or f'id{user_id}'))} (ID: <code>{int(user_id)}</code>)",
                f"🎯 <b>Аккаунт:</b> @{escape(str(target_username or ''))}",
                f"📦 <b>Товар:</b> {escape(amount_label)}",
                f"💰 <b>Сумма:</b> {final_rub:.2f}₽ / {final_usd:.2f}$",
                f"💠 <b>К оплате:</b> <code>{amount_ton:.6f} TON</code>",
                f"🏦 <b>Кошелёк магазина:</b> <code>{escape(merchant_wallet)}</code>",
                f"👛 <b>Кошелёк клиента:</b> <code>{escape(payer_wallet)}</code>",
                f"📝 <b>Комментарий:</b> <code>{escape(payment_memo)}</code>",
                "✅ <i>После поступления TON нажмите «Готово».</i>",
            ]
        )
        ...
        try:
            ...
            set_miniapp_purchase_history_error(
                history_id,
                f"Ошибка отправки TON-заявки в группу: {error}",
                status="error",
            )
            return miniapp_json_response(
                {
                    "ok": False,
                    "error": "Не удалось отправить TON-заявку в группу админов",
                    "errorCode": "MAPP-TON-GRP-001",
                },
                status=502,
            )
        ...
        return miniapp_json_response(
            {
                "ok": True,
                "message": "TON-заявка создана. Выполните перевод и дождитесь подтверждения админа.",
                "price": {
                    "rub": round(final_rub, 2),
                    "usd": final_usd,
                },
                ...
                "payment": {
                    "provider": "ton_wallet",
                    "paymentId": ton_payment_id,
                    "url": ton_links.get("tonkeeperLink") or "",
                    "deepLink": ton_links.get("deepLink") or "",
                    "wallet": merchant_wallet,
                    "payerWallet": payer_wallet,
                    "memo": payment_memo,
    ...
    if not is_freekassa_webhook_configured():
        config_error = "Webhook FreeKassa не настроен. Автовыдача отключена."
        set_miniapp_purchase_history_error(history_id, config_error, status="error")
        return miniapp_json_response(
            {
                "ok": False,
                "error": config_error,
                "errorCode": "MAPP-FK-WH-001",
                "operation": {
                    "id": operation_id,
                    "historyId": int(history_id),
                    "source": "miniapp",
...
async def handle_webapp_data(message: types.Message):
    if not MINIAPP_ALLOW_SENDDATA_FALLBACK:
        logging.warning(
            "Deprecated miniapp sendData payload blocked. user_id=%s",
            int(getattr(getattr(message, "from_user", None), "id", 0) or 0),
        )
        await message.answer(
            "⚠️ Обнаружен устаревший режим miniapp.\n"
            "Закройте мини-приложение и откройте его заново, затем нажмите «Оформить заказ» ещё раз."
        )
        return
...
async def confirm_delete_callback(callback: types.CallbackQuery):
    ...
    if history_id is not None:
        try:
            ...
            if finalize_result.get("ok"):
                history_details = get_miniapp_purchase_history_details(history_id)
                history_source = str(history_details.get("source") or "").strip().lower()
                if history_source in {"api_ton", "api_usdt_ton"}:
                    if history_source == "api_usdt_ton":
                        bill_id = f"usdt-ton-{int(history_id)}"
                        provider_name = "usdt_ton"
                    else:
                        bill_id = f"ton-{int(history_id)}"
                        provider_name = "ton_wallet"
                    save_payment(
                        int(history_details.get("userId") or 0),
                        bill_id,
                        round(float(history_details.get("priceRub") or 0.0), 2),
                        "paid",
                        (
                            f"miniapp payment paid, history_id={int(history_id)}, provider={provider_name}, "
                            f"confirmed_by_admin={int(getattr(callback.from_user, 'id', 0) or 0)}"
                        ),
                    )
                await miniapp_broadcast_event(
                    "purchase_finalized",
