...
(() => {
  ...
  const renderTonWalletButtons = () => {
    ...
    tonWalletButtons.forEach((button) => {
      const label = hasWallet ? `TON: ${shortWallet} • ${changeText}` : bindText;
  ...
  const bindTonWalletInteractive = async () => {
    ...
    const promptText = isEnglishUi()
      ? "Paste your TON wallet address"
      : "Вставьте адрес TON-кошелька";
    const input = window.prompt(promptText, boundTonWallet || "");
    if (input === null) {
      return;
    }
    const nextWallet = normalizeTonWalletAddress(input);
    if (!nextWallet) {
      showMessage(isEnglishUi() ? "Wallet is required" : "Введите TON-кошелёк");
      return;
    }
    if (!isValidTonWalletAddress(nextWallet)) {
      showMessage(isEnglishUi() ? "Invalid TON wallet" : "Некорректный TON-кошелёк");
      return;
    }
    ...
    try {
      ...
      const data = await response.json().catch(() => ({}));
      if (!response.ok || !data?.ok) {
        showMessage(data?.error || "Не удалось привязать TON-кошелёк");
        return;
      }
      const storedWallet = normalizeTonWalletAddress(data?.user?.tonWallet || nextWallet);
      persistTonWallet(storedWallet);
      renderTonWalletButtons();
  ...
  window.addEventListener("starslix:language-updated", renderTonWalletButtons);
  renderTonWalletButtons();
...
(() => {
  ...
  const translations = {
    ru: {
      ...
      paymentMethodText: "TON-кошелёк (прямой перевод)",
    ...
    en: {
      ...
      paymentMethodText: "TON wallet (direct transfer)",
