...
(() => {
  ...
  const renderHistory = () => {
    ...
    visibleItems.forEach((item, index) => {
      ...
      details.innerHTML = `
        <div class="profile-history-details-row"><span class="profile-history-details-label">${isEnglishLocale() ? "Operation ID" : "ID операции"}</span><span class="profile-history-details-value">${escapeHtml(operationId)}</span></div>
        <div class="profile-history-details-row"><span class="profile-history-details-label">${isEnglishLocale() ? "Product" : "Товар"}</span><span class="profile-history-details-value">${escapeHtml(typeTitle)}</span></div>
        <div class="profile-history-details-row"><span class="profile-history-details-label">${isEnglishLocale() ? "Quantity" : "Количество"}</span><span class="profile-history-details-value">${escapeHtml(amountText || "—")}</span></div>
        <div class="profile-history-details-row"><span class="profile-history-details-label">${isEnglishLocale() ? "Price" : "Стоимость"}</span><span class="profile-history-details-value">${escapeHtml(String(item.price || "—"))}</span></div>
        <div class="profile-history-details-row"><span class="profile-history-details-label">${isEnglishLocale() ? "Promo" : "Промокод"}</span><span class="profile-history-details-value">${escapeHtml(promoText)}</span></div>
        <div class="profile-history-details-row"><span class="profile-history-details-label">${isEnglishLocale() ? "Status" : "Статус"}</span><span class="profile-history-details-value">${escapeHtml(getHistoryStatusLabel(status))}</span></div>
        <div class="profile-history-details-row"><span class="profile-history-details-label">${isEnglishLocale() ? "Date" : "Дата и время"}</span><span class="profile-history-details-value">${escapeHtml(dateText)}</span></div>
        <div class="profile-history-details-row"><span class="profile-history-details-label">${isEnglishLocale() ? "Buyer" : "Покупатель"}</span><span class="profile-history-details-value">${escapeHtml(buyerText)}</span></div>
        <div class="profile-history-details-row"><span class="profile-history-details-label">${isEnglishLocale() ? "Recipient" : "Получатель"}</span><span class="profile-history-details-value">${escapeHtml(targetUsername)}</span></div>
        <div class="profile-history-details-row"><span class="profile-history-details-label">${isEnglishLocale() ? "Channel" : "Канал"}</span><span class="profile-history-details-value">${escapeHtml(channelText)}</span></div>
      `;
      ...
      row.append(iconWrap, main, statusNode, side, details);
  ...
  const submitOrder = async (button, panelView) => {
    ...
    try {
      ...
      if (!response.ok || !data?.ok) {
        const canFallbackToSendData = !response.ok && response.status >= 500;
        if (canFallbackToSendData && trySendViaWebAppData()) {
          return;
        }
...
(() => {
    ...
    const renderUsersRows = (items) => {
      ...
      usersTableBody.innerHTML = items
        .map((item) => {
          ...
          const userIdText = safeUserId > 0 ? escapeHtml(String(safeUserId)) : "—";
          const fullNameText = escapeHtml(String(item?.fullName || "").trim());
          const rowAriaLabel = fullNameText
            ? `Открыть профиль ${usernameText} (${fullNameText})`
            : `Открыть профиль ${usernameText}`;
          return `<tr class="admin-user-row" data-user-id="${safeUserId}" tabindex="0" role="button" aria-label="${rowAriaLabel}"><td>${usernameText}</td><td>${userIdText}</td></tr>`;
        })
        .join("");
  ...
  const renderOperationsSection = async () => {
    adminSectionContent.innerHTML = `
      <div class="admin-split admin-split--search">
        <div class="admin-search-wrap">
          <input class="admin-input admin-input--search" id="adminOperationsSearchInput" type="text" placeholder="Поиск по ID операции, @юзу, ID, промокоду">
    ...
    const renderRows = (items) => {
      ...
      tableBody.innerHTML = items
        .map((item) => {
          const operationId = escapeHtml(String(item?.operationId || "—"));
          ...
          return `<tr><td><code>${operationId}</code></td><td>${dateText}</td><td>${buyerText}</td><td>${targetText}</td><td>${itemText}</td><td>${priceText}</td><td>${statusBadge}</td><td>${promoText}</td><td>${channelText}</td></tr>`;
        })
        .join("");
    };
