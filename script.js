...
(() => {
  ...
  const renderStatsSection = async () => {
    adminSectionContent.innerHTML = `
      <div class="admin-panel-actions">
        ...
        <button class="admin-action-button" id="adminStatsShowUsersButton" type="button">Показать пользователей</button>
      ...
      <div class="admin-table-wrap" id="adminStatsUsersTableWrap" hidden>
        <table class="admin-table admin-table--compact">
          ...
          <tbody id="adminStatsUsersTableBody"><tr><td colspan="2">Нажмите «Показать пользователей»</td></tr></tbody>
        </table>
      </div>
      ...
      <div class="admin-result-card" id="adminStatsResult">Нажмите кнопку, чтобы получить данные.</div>
    `;
    ...
    const showUsersButton = document.getElementById("adminStatsShowUsersButton");
    ...
    const setButtonsState = (disabled) => {
      ...
      showUsersButton.disabled = disabled;
    ...
    const loadUsersPage = async ({ page, query }) => {
      ...
      try {
        ...
        resultNode.textContent = `Пользователей: ${formatNumber(Number.isFinite(safeTotal) ? safeTotal : 0)}`;
      } catch (error) {
    ...
    usersButton.addEventListener("click", async () => {
      ...
      try {
        const data = await adminRequest("stats_users");
        resultNode.textContent = `Всего пользователей: ${formatNumber(data.totalUsers)}`;
      } catch (error) {
        resultNode.textContent = error.message;
      } finally {
  ...
  const renderPromosSection = async () => {
    adminSectionContent.innerHTML = `
      <div class="admin-form-grid">
        ...
        <label class="admin-form-field"><span class="admin-form-label">Скидка %</span><input class="admin-input" id="adminPromoDiscountInput" type="number" min="1" max="100" step="1" value="10"></label>
        ...
        <label class="admin-form-field"><span class="admin-form-label">Макс. бесплатных звёзд</span><input class="admin-input" id="adminPromoFreeStarsInput" type="number" min="0" step="1" value="0"></label>
        <label class="admin-form-field"><span class="admin-form-label">Применяется к</span><select class="admin-select" id="adminPromoTargetInput"><option value="stars">Звёзды</option><option value="premium">Premium</option><option value="all">Всё</option></select></label>
      </div>
      <div class="admin-panel-actions">
      ...
      <div class="admin-table-wrap">
        <table class="admin-table">
          <thead><tr><th>Код</th><th>Скидка</th><th>Остаток</th><th>Истекает</th><th>Тип</th></tr></thead>
      ...
      <div class="admin-result-card" id="adminPromoResult">Заполните поля и нажмите «Создать промокод».</div>
    `;

    const codeInput = document.getElementById("adminPromoCodeInput");
    const discountInput = document.getElementById("adminPromoDiscountInput");
    ...
    const freeStarsInput = document.getElementById("adminPromoFreeStarsInput");
      ...
      try {
        ...
        tableBody.innerHTML = promos
          .map((promo) => {
            ...
            return `<tr><td><code>${escapeHtml(promo.code || "")}</code></td><td>${escapeHtml(String(promo.discount || 0))}%</td><td>${escapeHtml(String(remaining))}</td><td>${escapeHtml(String(promo.expiresAt || "—"))}</td><td>${escapeHtml(String(promo.target || "stars"))}</td></tr>`;
          })
          .join("");
      } catch (error) {
    ...
    createButton.addEventListener("click", async () => {
      ...
      try {
        const payload = {
          ...
          discount: Number.parseInt(discountInput.value, 10),
          ...
          maxFreeStars: Number.parseInt(freeStarsInput.value, 10),
          target: targetInput.value,
        };
        await adminRequest("promo_create", payload);
        resultNode.textContent = `Промокод ${payload.code || "—"} создан`;
        await loadPromos();
      } catch (error) {
        resultNode.textContent = error.message;
      } finally {
  ...
  const renderStarsSection = async () => {
    adminSectionContent.innerHTML = `
      ...
      <div class="admin-result-card" id="adminStarsResult">Используйте действия выше.</div>
    `;
    ...
    adminSectionContent.innerHTML = `
      ...
      <div class="admin-table-wrap">
        <table class="admin-table">
          ...
          <tbody id="adminPaymentHistoryBody"><tr><td colspan="5">Нажмите «История пополнений»</td></tr></tbody>
        </table>
      </div>
      <div class="admin-result-card" id="adminPaymentResult">Используйте действия выше.</div>
    `;
  ...
  const renderAddStarsSection = async () => {
    adminSectionContent.innerHTML = `
      ...
      <div class="admin-result-card" id="adminGrantResult">Заполните поля и нажмите кнопку.</div>
    `;
