...
(() => {
  ...
  const resolvePhotoUrl = (value) => {
    ...
    const baseUrl = buildApiUrl(normalized);
    ...
    try {
      ...
      if (!/\/api\/miniapp\/support\/photo\//.test(urlObj.pathname)) {
        return baseUrl;
      }
      if (!urlObj.searchParams.has("initData")) {
  ...
  const updateSupportBadge = () => {
    ...
    const rawCount = Number.parseInt(String(cachedSupportState?.userUnreadCount || "0"), 10);
    const count = Number.isFinite(rawCount) ? rawCount : 0;
    const shouldShow = hasChat && hasAdminMessages && count > 0;
    supportFabBadge.classList.toggle("is-active", shouldShow);
    supportFabBadge.hidden = !shouldShow;
    if (!shouldShow) {
  ...
  if (supportFabBadge) {
    supportFabBadge.classList.remove("is-active");
...
(() => {
  ...
  const normalizeHistoryEntry = (item) => {
    ...
    const explicitErrorCode = String(item?.errorCode || item?.error_code || "")
      .trim()
      .toUpperCase();
    const promoErrorCodeMatch = promoError.match(/\[code=([A-Z0-9\-]+)\]/i);
    const errorCode = explicitErrorCode || (promoErrorCodeMatch ? String(promoErrorCodeMatch[1] || "").toUpperCase() : "");
  ...
  const openHistoryOperationModal = (payload) => {
    ...
    const errorCodeText = String(payload?.errorCode || "—").trim().toUpperCase() || "—";
    ...
    if (errorCodeText !== "—" || errorText !== "—") {
      rows.push([isEnglishLocale() ? "Error code" : "Код ошибки", errorCodeText]);
      rows.push([isEnglishLocale() ? "Diagnostics" : "Диагностика", errorText]);
    }
  ...
  const renderHistory = () => {
    ...
    visibleItems.forEach((item, index) => {
      ...
      const errorText = String(item?.promoError || "").trim();
  ...
  const addCurrentOrderToHistory = (override = null) => {
    ...
    const entryErrorCodeOverride = String(override?.errorCode || "").trim().toUpperCase();
    const entryErrorCodeMatch = entryPromoError.match(/\[code=([A-Z0-9\-]+)\]/i);
    const entryErrorCode =
      entryErrorCodeOverride || (entryErrorCodeMatch ? String(entryErrorCodeMatch[1] || "").toUpperCase() : "");
...
(() => {
  ...
  const connectMiniappEvents = () => {
    ...
    eventsSource.addEventListener("purchase_failed", (event) => {
      const payload = parseEventPayload(event);
      window.dispatchEvent(
        new CustomEvent("starslix:server-update", {
          detail: payload || {},
  ...
  const getFallbackUsername = () => {
    const usernameRaw = String(window.Telegram?.WebApp?.initDataUnsafe?.user?.username || "").trim();
    if (!usernameRaw) {
      return "";
    }
  ...
  const tonWalletStorageKey = "starslix:ton-wallet";
  const tonWalletButtons = Array.from(document.querySelectorAll("[data-bind-ton-wallet]"));
  const tonWalletUnbindButtons = Array.from(document.querySelectorAll("[data-unbind-ton-wallet]"));
  const profileTonWalletCard = document.getElementById("profileTonWalletCard");
  const profileTonWalletTitle = document.getElementById("profileTonWalletTitle");
  const profileTonWalletStatus = document.getElementById("profileTonWalletStatus");
  const profileTonWalletAddress = document.getElementById("profileTonWalletAddress");
  const tonWalletUnbindModal = document.getElementById("tonWalletUnbindModal");
  const tonWalletUnbindTitle = document.getElementById("tonWalletUnbindTitle");
  const tonWalletUnbindCloseButton = document.getElementById("tonWalletUnbindCloseButton");
  const tonWalletUnbindConfirmButton = document.getElementById("tonWalletUnbindConfirmButton");
  const tonWalletUnbindText = document.getElementById("tonWalletUnbindText");
  const normalizeTonWalletAddress = (value) => String(value || "").trim();
  const isValidTonWalletAddress = (value) => {
    const wallet = normalizeTonWalletAddress(value);
    if (!wallet) {
      return false;
    }
    if (/^(?:-1|0):[0-9a-fA-F]{64}$/.test(wallet)) {
      return true;
    }
    return /^[A-Za-z0-9_-]{48,128}$/.test(wallet);
  };
  const getShortTonWallet = (value) => {
    const wallet = normalizeTonWalletAddress(value);
    if (!wallet) {
      return "";
    }
  ...
  const isEnglishUi = () =>
    String(document.documentElement.lang || "ru").toLowerCase().startsWith("en");

  const paymentMethodStorageKey = "starslix:payment-method";
  const paymentSelectNodes = Array.from(document.querySelectorAll("[data-payment-select]"));
  const normalizePaymentMethod = (value) => {
    const raw = String(value || "")
      .trim()
      .toLowerCase();
    if (raw === "ton" || raw === "tonwallet" || raw === "ton_wallet") {
      return "ton_wallet";
    }
    if (raw === "usdt_ton" || raw === "usdtton" || raw === "ton_usdt") {
      return "usdt_ton";
    }
    return "44";
  };
  const getTonMethodStatusLabel = () =>
    boundTonWallet
      ? "TON"
      : isEnglishUi()
        ? "TON (wallet not linked)"
        : "TON (кошелёк не привязан)";
  const getPaymentMethodLabels = () =>
    isEnglishUi()
      ? {
          "44": "SBP #1",
          ton_wallet: getTonMethodStatusLabel(),
          usdt_ton: "USDT TON",
        }
      : {
          "44": "СБП #1",
  ...
  const closeAllPaymentMethodMenus = () => {
    paymentSelectNodes.forEach((node) => {
      node.classList.remove("is-open");
      const toggleNode = node.querySelector("[data-payment-select-toggle]");
      const menuNode = node.querySelector("[data-payment-select-menu]");
  ...
  let selectedPaymentMethod = "44";
  try {
    selectedPaymentMethod = normalizePaymentMethod(
      window.localStorage.getItem(paymentMethodStorageKey) || "44"
    );
  } catch {
    selectedPaymentMethod = "44";
  }

  const persistPaymentMethod = (method) => {
    selectedPaymentMethod = normalizePaymentMethod(method);
    try {
      window.localStorage.setItem(paymentMethodStorageKey, selectedPaymentMethod);
    } catch {}
  };

  const isTonWalletRequiredMethod = (method) => {
    const normalized = normalizePaymentMethod(method);
    return normalized === "ton_wallet" || normalized === "usdt_ton";
  };

  const updateTonWalletRequirementUi = () => {
    const walletMissing = isTonWalletRequiredMethod(selectedPaymentMethod) && !boundTonWallet;
    const connectButtonText = isEnglishUi() ? "Connect your wallet" : "Подключить свой кошелёк";
    const orderButtonText = isEnglishUi() ? "Place order" : "Оформить заказ";
    const alertText = isEnglishUi()
      ? "Connect your TON wallet to continue"
      : "Подключите свой TON-кошелёк, чтобы продолжить";

    document
      .querySelectorAll('.panel-view[data-view="stars"], .panel-view[data-view="premium"]')
      .forEach((panelView) => {
        const alertNode = panelView.querySelector("[data-wallet-required-alert]");
        const alertTextNode = panelView.querySelector("[data-wallet-required-alert-text]");
        const submitButton = panelView.querySelector(".order-submit-button");

        if (alertNode instanceof HTMLElement) {
          alertNode.hidden = !walletMissing;
        }
        if (alertTextNode instanceof HTMLElement) {
          alertTextNode.textContent = alertText;
        }
        if (submitButton instanceof HTMLButtonElement) {
          submitButton.setAttribute("data-wallet-connect-mode", walletMissing ? "1" : "0");
          if (!submitButton.classList.contains("is-loading")) {
            submitButton.textContent = walletMissing ? connectButtonText : orderButtonText;
          }
        }
      });
  };

  const renderPaymentMethodSelects = () => {
    const labels = getPaymentMethodLabels();
    const selectedLabel = labels[selectedPaymentMethod] || labels["44"];
    paymentSelectNodes.forEach((node) => {
      const currentNode = node.querySelector("[data-payment-select-current]");
      if (currentNode instanceof HTMLElement) {
        currentNode.textContent = selectedLabel;
      }
      node.querySelectorAll("[data-payment-method]").forEach((optionNode) => {
        const optionMethod = normalizePaymentMethod(optionNode.getAttribute("data-payment-method") || "");
        const optionLabelNode = optionNode.querySelector("[data-payment-method-label]");
        const isDisabled =
          optionNode.hasAttribute("disabled") ||
          optionNode.getAttribute("data-payment-method-disabled") === "1";
        if (optionLabelNode instanceof HTMLElement) {
          optionLabelNode.textContent = labels[optionMethod] || labels["44"];
        }
        optionNode.classList.toggle("is-disabled", isDisabled);
        optionNode.classList.toggle("is-active", optionMethod === selectedPaymentMethod);
      });
    });
    updateTonWalletRequirementUi();
  };

  paymentSelectNodes.forEach((node) => {
    const toggleNode = node.querySelector("[data-payment-select-toggle]");
    const menuNode = node.querySelector("[data-payment-select-menu]");
    if (!(toggleNode instanceof HTMLElement) || !(menuNode instanceof HTMLElement)) {
      return;
    }
    toggleNode.addEventListener("click", (event) => {
      event.preventDefault();
      const isOpen = node.classList.contains("is-open");
      closeAllPaymentMethodMenus();
      if (isOpen) {
        return;
      }
    ...
    node.querySelectorAll("[data-payment-method]").forEach((optionNode) => {
      optionNode.addEventListener("click", (event) => {
        ...
        if (isDisabled) {
          showMessage(
            isEnglishUi()
              ? "USDT TON method is not available yet."
              : "Метод USDT TON пока недоступен."
          );
          closeAllPaymentMethodMenus();
          return;
        }
        const nextMethod = normalizePaymentMethod(optionNode.getAttribute("data-payment-method") || "");
        persistPaymentMethod(nextMethod);
        closeAllPaymentMethodMenus();
        renderPaymentMethodSelects();
        renderTonWalletButtons();
        if (isTonWalletRequiredMethod(nextMethod) && !boundTonWallet) {
          showMessage(
            isEnglishUi()
              ? "To pay via TON/USDT TON, tap Connect wallet under the price."
              : "Для оплаты через TON/USDT TON нажмите «Подключить свой кошелёк» под ценой."
          );
        }
        window.dispatchEvent(
          new CustomEvent("starslix:payment-method-updated", {
            detail: { method: selectedPaymentMethod },
          })
        );
      });
    });
  ...
  document.addEventListener("click", (event) => {
    ...
    if (target.closest("[data-payment-select]")) {
      return;
    }
    closeAllPaymentMethodMenus();
  });
  ...
  const getTonConnectUi = (() => {
    ...
    return () => {
      ...
      try {
        ...
        instance = new TonConnectUIClass({
          manifestUrl,
          uiPreferences: {
            theme: "DARK",
          },
  ...
  const requestTonConnectWallet = async (tonConnectUi) => {
    if (!tonConnectUi) {
      return "";
    }

    let currentWallet = getWalletFromTonConnectData(tonConnectUi.wallet);
    if (isValidTonWalletAddress(currentWallet)) {
      return currentWallet;
    }

    const canUseModalFlow =
      typeof tonConnectUi.openModal === "function" &&
    ...
    if (canUseModalFlow) {
      await new Promise((resolve, reject) => {
        ...
        timeoutId = window.setTimeout(() => {
          finishReject(new Error("TON connect timeout"));
        }, 90000);
        Promise.resolve(tonConnectUi.openModal())
          .then(() => {})
          .catch((error) => {
            finishReject(error);
          });
      });
      currentWallet = getWalletFromTonConnectData(tonConnectUi.wallet);
      if (isValidTonWalletAddress(currentWallet)) {
        return currentWallet;
      }
      return "";
    ...
    if (typeof tonConnectUi.connectWallet === "function") {
      try {
        ...
        currentWallet = getWalletFromTonConnectData(connectedWallet);
        if (!isValidTonWalletAddress(currentWallet)) {
          currentWallet = getWalletFromTonConnectData(tonConnectUi.wallet);
        }
        if (isValidTonWalletAddress(currentWallet)) {
  ...
  const getWalletFromTonConnectData = (value) =>
    normalizeTonWalletAddress(
      value?.account?.address ||
        value?.address ||
        getTonConnectUi()?.wallet?.account?.address ||
        getTonConnectUi()?.account?.address ||
        ""
    );

  let boundTonWallet = "";
  try {
    boundTonWallet = normalizeTonWalletAddress(window.localStorage.getItem(tonWalletStorageKey) || "");
  } catch {
    boundTonWallet = "";
  }
  if (!isValidTonWalletAddress(boundTonWallet)) {
    boundTonWallet = "";
  }

  const persistTonWallet = (wallet) => {
    const normalized = normalizeTonWalletAddress(wallet);
    if (!isValidTonWalletAddress(normalized)) {
      return;
    }
    boundTonWallet = normalized;
    try {
      window.localStorage.setItem(tonWalletStorageKey, boundTonWallet);
    } catch {}
  };

  const clearTonWallet = () => {
    boundTonWallet = "";
    try {
      window.localStorage.removeItem(tonWalletStorageKey);
    } catch {}
  };

  const renderTonWalletProfileCard = () => {
    if (!(profileTonWalletCard instanceof HTMLElement)) {
      return;
    }
    const hasWallet = Boolean(boundTonWallet);
    profileTonWalletCard.setAttribute("data-has-wallet", hasWallet ? "1" : "0");
    if (profileTonWalletTitle instanceof HTMLElement) {
      profileTonWalletTitle.textContent = isEnglishUi() ? "TON wallet" : "TON-кошелёк";
    }
    if (profileTonWalletStatus instanceof HTMLElement) {
      profileTonWalletStatus.textContent = hasWallet
        ? isEnglishUi()
          ? "Linked"
          : "Привязан"
        : isEnglishUi()
    ...
    if (profileTonWalletAddress instanceof HTMLElement) {
      profileTonWalletAddress.textContent = hasWallet
        ? boundTonWallet
        : isEnglishUi()
          ? "Link your wallet to pay via TON."
          : "Привяжите кошелёк для оплаты через TON.";
      profileTonWalletAddress.setAttribute("title", hasWallet ? boundTonWallet : "");
    }
  };

  const renderTonWalletUnbindModalText = () => {
    if (tonWalletUnbindTitle instanceof HTMLElement) {
      tonWalletUnbindTitle.textContent = isEnglishUi()
        ? "Unbind TON wallet?"
        : "Отвязать TON-кошелёк?";
    }
    if (tonWalletUnbindCloseButton instanceof HTMLButtonElement) {
      tonWalletUnbindCloseButton.textContent = isEnglishUi() ? "Close" : "Закрыть";
    }
    if (tonWalletUnbindConfirmButton instanceof HTMLButtonElement) {
      tonWalletUnbindConfirmButton.textContent = isEnglishUi() ? "Unbind" : "Отвязать";
    }
    if (tonWalletUnbindText instanceof HTMLElement) {
      tonWalletUnbindText.textContent = isEnglishUi()
        ? "TON payment will be unavailable until you link a wallet again."
        : "Оплата через TON станет недоступна, пока вы снова не привяжете кошелёк.";
    }
  };

  const renderTonWalletButtons = () => {
    const hasWallet = Boolean(boundTonWallet);
    const tonMethodActive = isTonWalletRequiredMethod(selectedPaymentMethod);
    const bindText = isEnglishUi() ? "Bind TON wallet" : "Привязать TON-кошелёк";
    const changeText = isEnglishUi() ? "Change wallet" : "Изменить кошелёк";
    const unbindText = isEnglishUi() ? "Unbind TON wallet" : "Отвязать TON-кошелёк";
    const shortWallet = getShortTonWallet(boundTonWallet);
    tonWalletButtons.forEach((button) => {
      const inProfile = String(button.getAttribute("data-ton-wallet-mode") || "") === "profile";
      const label = inProfile
        ? hasWallet
          ? changeText
          : bindText
        : hasWallet
          ? `TON: ${shortWallet} • ${changeText}`
          : bindText;
      button.textContent = label;
      button.hidden = inProfile ? false : !tonMethodActive;
      button.setAttribute("data-has-wallet", hasWallet ? "1" : "0");
      button.setAttribute("title", hasWallet ? boundTonWallet : bindText);
    });
    tonWalletUnbindButtons.forEach((button) => {
      const inProfile = String(button.getAttribute("data-ton-wallet-mode") || "") === "profile";
      button.textContent = unbindText;
      button.hidden = inProfile ? !hasWallet : !(tonMethodActive && hasWallet);
      button.setAttribute("title", hasWallet ? boundTonWallet : unbindText);
    });
    renderTonWalletProfileCard();
    renderTonWalletUnbindModalText();
    updateTonWalletRequirementUi();
  };

  const fetchBoundTonWalletFromApi = async () => {
    const initData = String(window.Telegram?.WebApp?.initData || "").trim();
    if (!initData) {
      return;
    }
    try {
      ...
      const apiWallet = normalizeTonWalletAddress(data?.user?.tonWallet || "");
      if (isValidTonWalletAddress(apiWallet)) {
        persistTonWallet(apiWallet);
      } else {
        clearTonWallet();
      }
      renderTonWalletButtons();
  ...
  const saveTonWalletToApi = async (walletAddress) => {
    const initData = String(window.Telegram?.WebApp?.initData || "").trim();
    if (!initData) {
      return { ok: false, error: "Открой мини-приложение через Telegram" };
    }
    const nextWallet = normalizeTonWalletAddress(walletAddress);
    if (!isValidTonWalletAddress(nextWallet)) {
      return { ok: false, error: isEnglishUi() ? "Invalid TON wallet" : "Некорректный TON-кошелёк" };
    }
    const response = await fetch(buildApiUrl("/api/miniapp/profile"), {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        initData,
        action: "bind_ton_wallet",
        wallet: nextWallet,
      }),
    });
    const data = await response.json().catch(() => ({}));
    if (!response.ok || !data?.ok) {
      return { ok: false, error: data?.error || "Не удалось привязать TON-кошелёк" };
    }
    const storedWallet = normalizeTonWalletAddress(data?.user?.tonWallet || nextWallet);
    if (!isValidTonWalletAddress(storedWallet)) {
      return { ok: false, error: "Сервер вернул некорректный адрес кошелька" };
    }
    return { ok: true, wallet: storedWallet };
  };

  let isTonWalletBindingInProgress = false;
  const bindTonWalletInteractive = async () => {
    if (isTonWalletBindingInProgress) {
      return;
    }
    const initData = String(window.Telegram?.WebApp?.initData || "").trim();
    if (!initData) {
      showMessage("Открой мини-приложение через Telegram");
      return;
    }
    isTonWalletBindingInProgress = true;
    ...
    try {
      ...
      if (!tonConnectUi) {
        showMessage(
          isEnglishUi()
            ? "TonConnect is unavailable. Reopen the mini app."
            : "TonConnect недоступен. Перезапустите мини-приложение."
        );
        return;
      }
      if (boundTonWallet && typeof tonConnectUi.disconnect === "function") {
        try {
          await tonConnectUi.disconnect();
        } catch {}
      }
      const nextWallet = await requestTonConnectWallet(tonConnectUi);

      if (!isValidTonWalletAddress(nextWallet)) {
        showMessage(
          isEnglishUi()
            ? "Wallet was not connected. Choose a wallet and try again."
            : "Кошелёк не подключён. Выберите кошелёк и попробуйте снова."
        );
        return;
      }

      const bindResult = await saveTonWalletToApi(nextWallet);
      if (!bindResult.ok) {
        showMessage(bindResult.error || "Не удалось привязать TON-кошелёк");
        return;
      }

      persistTonWallet(bindResult.wallet);
      renderTonWalletButtons();
      renderPaymentMethodSelects();
      showMessage(isEnglishUi() ? "TON wallet linked" : "TON-кошелёк привязан");
    } catch (error) {
      const message = String(error?.message || "").toLowerCase();
      if (message.includes("rejected") || message.includes("cancel")) {
        return;
      }
      showMessage(`Ошибка сети: ${error?.message || "unknown error"}`);
    } finally {
      tonWalletButtons.forEach((button) => {
      ...
      isTonWalletBindingInProgress = false;
    }
  };

  let isTonWalletUnbindModalOpen = false;
  const openTonWalletUnbindModal = () => {
    if (!(tonWalletUnbindModal instanceof HTMLElement)) {
      return;
    }
    if (isTonWalletUnbindModalOpen) {
      return;
    }
    isTonWalletUnbindModalOpen = true;
    tonWalletUnbindModal.hidden = false;
    tonWalletUnbindModal.setAttribute("aria-hidden", "false");
    document.body.classList.add("is-ton-wallet-unbind-open");
    window.requestAnimationFrame(() => {
      tonWalletUnbindModal.classList.add("is-open");
    });
  };

  const closeTonWalletUnbindModal = () => {
    if (!(tonWalletUnbindModal instanceof HTMLElement)) {
      return;
    }
    isTonWalletUnbindModalOpen = false;
    tonWalletUnbindModal.classList.remove("is-open");
    tonWalletUnbindModal.setAttribute("aria-hidden", "true");
    document.body.classList.remove("is-ton-wallet-unbind-open");
    window.setTimeout(() => {
      if (!isTonWalletUnbindModalOpen) {
        tonWalletUnbindModal.hidden = true;
      }
    }, 180);
  };

  const unbindTonWalletInteractive = async () => {
    const initData = String(window.Telegram?.WebApp?.initData || "").trim();
    if (!initData) {
      showMessage("Открой мини-приложение через Telegram");
      return;
    ...
    try {
      const response = await fetch(buildApiUrl("/api/miniapp/profile"), {
        ...
        body: JSON.stringify({
          initData,
          action: "unbind_ton_wallet",
        }),
      });
      const data = await response.json().catch(() => ({}));
      if (!response.ok || !data?.ok) {
        showMessage(data?.error || "Не удалось отвязать TON-кошелёк");
        return;
      }
      const tonConnectUi = getTonConnectUi();
      ...
      renderTonWalletButtons();
      renderPaymentMethodSelects();
      showMessage(isEnglishUi() ? "TON wallet unbound" : "TON-кошелёк отвязан");
    } catch (error) {
      showMessage(`Ошибка сети: ${error?.message || "unknown error"}`);
    } finally {
      tonWalletButtons.forEach((button) => {
  ...
  tonWalletUnbindButtons.forEach((button) => {
    button.addEventListener("click", (event) => {
      ...
      openTonWalletUnbindModal();
    });
  });
  tonWalletUnbindCloseButton?.addEventListener("click", (event) => {
    event.preventDefault();
    closeTonWalletUnbindModal();
  });
  tonWalletUnbindConfirmButton?.addEventListener("click", (event) => {
    event.preventDefault();
    closeTonWalletUnbindModal();
    unbindTonWalletInteractive();
  });
  tonWalletUnbindModal?.addEventListener("click", (event) => {
    const target = event.target;
    if (!(target instanceof Element)) {
      return;
    }
    if (target.closest("[data-ton-wallet-unbind-close]")) {
      closeTonWalletUnbindModal();
    }
  });
  document.addEventListener("keydown", (event) => {
    if (event.key === "Escape" && isTonWalletUnbindModalOpen) {
      closeTonWalletUnbindModal();
    }
  });
  window.addEventListener("starslix:screen-changed", (event) => {
    const screen = String(event?.detail?.screen || "");
    if (screen === "profile") {
      fetchBoundTonWalletFromApi().catch(() => {});
    }
    if (isTonWalletUnbindModalOpen) {
      closeTonWalletUnbindModal();
    }
  ...
  const getOrderPayload = (panelView, options = {}) => {
    ...
    const username = usernameFromInput || (previewOnly ? getFallbackUsername() : "");
    ...
    const walletPaymentSelected = isTonWalletRequiredMethod(paymentMethod);
    if (!previewOnly && walletPaymentSelected && !boundTonWallet) {
      return { error: "Сначала привяжите TON-кошелёк" };
    }
    const initData = window.Telegram?.WebApp?.initData || "";
    ...
    return {
      ...
      amount,
      promoCode,
      tonWallet: walletPaymentSelected ? boundTonWallet : "",
      paymentMethod,
      initData,
  ...
  const promoPreviewState = new WeakMap();

  const getPromoPreviewState = (panelView) => {
    if (!panelView) {
      return null;
    }
    let state = promoPreviewState.get(panelView);
    if (!state) {
      state = {
        timerId: 0,
        requestSeq: 0,
      };
      promoPreviewState.set(panelView, state);
    }
    return state;
  };

  const restoreBasePanelPrice = (panelView) => {
    const amountInput = panelView?.querySelector('.form-input[type="number"]');
  ...
  const requestPromoPreview = async (panelView) => {
    const promoInput = panelView?.querySelector(".form-input--promo");
    const priceUsdNode = panelView?.querySelector("[data-price-usd]");
    const priceRubNode = panelView?.querySelector("[data-price-rub]");
    if (
      !(promoInput instanceof HTMLInputElement) ||
      !(priceUsdNode instanceof HTMLElement) ||
      !(priceRubNode instanceof HTMLElement)
    ) {
      return;
    }
    ...
    if (!promoCode) {
      restoreBasePanelPrice(panelView);
      return;
    }

    const payload = getOrderPayload(panelView, { previewOnly: true });
    if (!payload || payload.error) {
      return;
    }

    const state = getPromoPreviewState(panelView);
    if (!state) {
      return;
    }
    ...
    try {
      ...
      if (!response.ok || !data?.ok || !data?.price) {
        return;
      }

      const nextUsd = Number(data.price.usd);
      const nextRub = Number(data.price.rub);
      if (Number.isFinite(nextUsd)) {
        priceUsdNode.textContent = formatUsd(nextUsd);
      }
      if (Number.isFinite(nextRub)) {
        priceRubNode.textContent = formatRub(nextRub);
      }
    } catch {}
  };

  const schedulePromoPreview = (panelView, immediate = false) => {
    const state = getPromoPreviewState(panelView);
    if (!state) {
      return;
    }
    ...
    const run = () => {
      requestPromoPreview(panelView).catch(() => {});
    };
    if (immediate) {
      run();
      return;
    }
    state.timerId = window.setTimeout(run, 220);
  };
    ...
    .forEach((panelView) => {
      ...
      if (promoInput instanceof HTMLInputElement) {
        promoInput.addEventListener("input", () => {
          schedulePromoPreview(panelView);
        });
      }
      ...
      panelView.querySelectorAll(".offer-card").forEach((card) => {
        card.addEventListener("click", () => {
          schedulePromoPreview(panelView, true);
        });
      });
    });

  window.addEventListener("starslix:payment-method-updated", () => {
    document
      .querySelectorAll('.panel-view[data-view="stars"], .panel-view[data-view="premium"]')
      .forEach((panelView) => {
        schedulePromoPreview(panelView, true);
  ...
  const submitOrder = async (button, panelView) => {
    ...
    try {
      ...
      if (!response.ok || !data?.ok) {
        ...
        showMessage(`Не удалось отправить заявку${errorCode ? ` [${errorCode}]` : ""}: ${details}`);
        return;
      }
      ...
      if (paymentUrl) {
        const paymentProvider = String(data?.payment?.provider || "").trim().toLowerCase();
        try {
          if (
            (paymentProvider === "ton_wallet" || paymentProvider === "usdt_ton") &&
            window.Telegram?.WebApp?.openLink
          ) {
            window.Telegram.WebApp.openLink(paymentUrl);
          } else {
            // Keep payment flow inside Telegram miniapp webview.
            window.location.assign(paymentUrl);
          }
        } catch {}
      } else {
        showMessage("Счёт создан, но ссылка оплаты не получена. Обратитесь в поддержку.");
      }

      clearPanelInputs(panelView);
      const promoErrorNotice = data?.promo?.error ? `\nПромокод: ${data.promo.error}` : "";
      showMessage(`Счёт создан. Перейдите к оплате${promoErrorNotice}`);
      return;
    } catch (error) {
      showMessage(
  ...
  document
    ...
    .forEach((button) => {
      button.addEventListener("click", (event) => {
        event.preventDefault();
        if (isTonWalletRequiredMethod(selectedPaymentMethod) && !boundTonWallet) {
          bindTonWalletInteractive();
          return;
        }
...
(() => {
  ...
  const sectionSubtitles = {
    ...
    procurement: "Закупочные цены SPEND с учётом комиссии",
  ...
  const renderProcurementSection = async () => {
    adminSectionContent.innerHTML = `
      <div class="admin-panel-actions">
        <button class="admin-action-button" id="adminProcurementRefreshButton" type="button">Обновить закупку</button>
      </div>
      <div class="admin-result-card" id="adminProcurementMeta">Загрузка закупочных цен...</div>
      <div class="admin-table-wrap">
        <table class="admin-table">
          <thead>
            <tr><th>Позиция</th><th>Кол-во в расчёте</th><th>Цена (TON)</th><th>За 1 ед. (TON)</th></tr>
          </thead>
          <tbody id="adminProcurementBody"><tr><td colspan="4">Загрузка...</td></tr></tbody>
        </table>
      </div>
      <div class="admin-result-card" id="adminProcurementResult"></div>
    `;

    const refreshButton = document.getElementById("adminProcurementRefreshButton");
    const metaNode = document.getElementById("adminProcurementMeta");
    const bodyNode = document.getElementById("adminProcurementBody");
    const resultNode = document.getElementById("adminProcurementResult");
    if (!refreshButton || !metaNode || !bodyNode || !resultNode) {
      return;
    }
    ...
    const renderRows = (itemsRaw) => {
      const items = itemsRaw && typeof itemsRaw === "object" ? itemsRaw : {};
      const rows = [
        { key: "star", title: "Stars" },
        { key: "premium3", title: "Premium 3 мес." },
        { key: "premium6", title: "Premium 6 мес." },
        { key: "premium12", title: "Premium 12 мес." },
        { key: "ton", title: "TON" },
      ];

      bodyNode.innerHTML = rows
        .map((row) => {
          const item = items[row.key] && typeof items[row.key] === "object" ? items[row.key] : {};
          const amountValue = Number.parseInt(String(item.amount || 0), 10);
          const tonPriceText = formatTonValue(item.tonPrice);
          const unitTonText = formatTonValue(item.unitTonPrice);
          const itemError = String(item.error || "").trim();
          const amountText = Number.isFinite(amountValue) && amountValue > 0 ? formatNumber(amountValue) : "—";
          const tonCellText = tonPriceText === "—" ? "—" : `${tonPriceText} TON`;
          const unitCellText = itemError
            ? `Ошибка: ${itemError}`
            : unitTonText === "—"
              ? "—"
              : `${unitTonText} TON`;
          return `<tr><td>${escapeHtml(row.title)}</td><td>${escapeHtml(amountText)}</td><td>${escapeHtml(tonCellText)}</td><td>${escapeHtml(unitCellText)}</td></tr>`;
        })
        .join("");
    };

    const loadProcurement = async () => {
      const data = await adminRequest("procurement_get");
      renderRows(data.items || {});
      const feeValue = Number(data.feePercent);
      const feeText = Number.isFinite(feeValue) && feeValue > 0
        ? `${Number.isInteger(feeValue) ? feeValue : feeValue.toFixed(2)}%`
        : "2%";
      const errorText = String(data.error || "").trim();
      metaNode.textContent = data.ok
        ? `Закупочные цены из SPEND API (fee ${feeText})`
        : errorText
          ? `Часть данных недоступна: ${errorText} (fee ${feeText})`
          : `Часть данных недоступна (fee ${feeText})`;
    };
    ...
    refreshButton.addEventListener("click", async () => {
      ...
      try {
        ...
        bodyNode.innerHTML = `<tr><td colspan="4">${escapeHtml(error.message)}</td></tr>`;
        resultNode.textContent = error.message;
      } finally {
    ...
    try {
      ...
      bodyNode.innerHTML = `<tr><td colspan="4">${escapeHtml(error.message)}</td></tr>`;
      metaNode.textContent = "Не удалось загрузить закупочные цены";
      resultNode.textContent = error.message;
    }
  };
  ...
  const renderStarsSection = async () => {
    adminSectionContent.innerHTML = `
      ...
      <div class="admin-result-card" id="adminMarketRatesResult">Курсы загружаются...</div>
      <div class="admin-form-grid">
    ...
    const marketRatesNode = document.getElementById("adminMarketRatesResult");
    ...
    const renderMarketRates = (marketRates) => {
      const rubPerTon = Number(marketRates?.rubPerTon);
      const rubPerStar = Number(marketRates?.rubPerStar);
      const sampleAmount = Number.parseInt(String(marketRates?.estimate?.amount || "0"), 10);
      const feePercent = Number(marketRates?.estimate?.fee);
      if (Number.isFinite(rubPerTon) && rubPerTon > 0 && Number.isFinite(rubPerStar) && rubPerStar > 0) {
        const tonText = Number.isInteger(rubPerTon) ? String(rubPerTon) : rubPerTon.toFixed(2);
        const starText = Number.isInteger(rubPerStar) ? String(rubPerStar) : rubPerStar.toFixed(3);
        const sampleText =
          Number.isFinite(sampleAmount) && sampleAmount > 0 ? `${formatNumber(sampleAmount)}⭐` : "100⭐";
        const feeText =
          Number.isFinite(feePercent) && feePercent > 0
            ? `${Number.isInteger(feePercent) ? feePercent : feePercent.toFixed(2)}%`
            : "2%";
        marketRatesNode.textContent = `Курс SPEND: 1 TON ≈ ${tonText}₽ | 1 ⭐ ≈ ${starText}₽ (расчет по ${sampleText}, fee ${feeText})`;
        return;
      }
      const errorText = String(marketRates?.error || "").trim();
      marketRatesNode.textContent = errorText
        ? `Курс SPEND недоступен: ${errorText}`
        : "Курс SPEND недоступен";
    };
    ...
    const loadData = async () => {
      ...
      renderMarketRates(data.marketRates || {});
    };
    ...
    try {
      ...
      marketRatesNode.textContent = "Не удалось загрузить курсы SPEND";
      resultNode.textContent = error.message;
    }
  };
  ...
  const renderAdminsSection = async () => {
    adminSectionContent.innerHTML = `
      <div class="admin-split admin-split--search">
        <div class="admin-search-wrap">
          <input class="admin-input admin-input--search" id="adminManageUserIdInput" type="text" inputmode="text" autocomplete="off" placeholder="ID или @username">
    ...
    let canManage = false;
    const getAdminTarget = () => String(userIdInput.value || "").trim();
    ...
    addButton.addEventListener("click", async () => {
      const userTarget = getAdminTarget();
      if (!userTarget) {
        resultNode.textContent = "Введите ID или @username";
        return;
      }
      setButtonsState(true);
      try {
        const data = await adminRequest("admin_add", { userId: userTarget });
        const resolvedId = Number.parseInt(String(data.userId || "0"), 10);
        const resultId = Number.isFinite(resolvedId) && resolvedId > 0 ? resolvedId : userTarget;
        renderTable(Array.isArray(data.items) ? data.items : []);
        resultNode.textContent = data.changed
          ? `Пользователь ${resultId} добавлен в админы`
          : `Пользователь ${resultId} уже админ`;
        userIdInput.value = "";
      } catch (error) {
        resultNode.textContent = error.message;
      } finally {
    ...
    removeButton.addEventListener("click", async () => {
      ...
      if (!window.confirm(`Убрать пользователя ${userTarget} из админов?`)) {
        return;
      }

      setButtonsState(true);
      try {
        const data = await adminRequest("admin_remove", { userId: userTarget });
        const resolvedId = Number.parseInt(String(data.userId || "0"), 10);
        const resultId = Number.isFinite(resolvedId) && resolvedId > 0 ? resolvedId : userTarget;
        renderTable(Array.isArray(data.items) ? data.items : []);
        resultNode.textContent = data.changed
          ? `Пользователь ${resultId} удалён из админов`
          : "Изменений нет";
        userIdInput.value = "";
      } catch (error) {
        resultNode.textContent = error.message;
