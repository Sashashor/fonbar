...
(() => {
  ...
  const supportModalExitButton = document.getElementById("supportModalExitButton");
  ...
  const supportModalInitialForm = document.getElementById("supportModalInitialForm");
  const supportInitialUsernameInput = document.getElementById("supportInitialUsernameInput");
  const supportInitialQuestionInput = document.getElementById("supportInitialQuestionInput");
  const supportInitialPhotoInput = document.getElementById("supportInitialPhotoInput");
  const supportInitialPhotoName = document.getElementById("supportInitialPhotoName");
  const supportInitialSendButton = document.getElementById("supportInitialSendButton");
  ...
  if (
    ...
    !supportModalExitButton ||
    !supportModalMeta ||
    !supportModalMessages ||
    !supportModalInitialForm ||
    !supportInitialUsernameInput ||
    !supportInitialQuestionInput ||
    !supportInitialPhotoInput ||
    !supportInitialPhotoName ||
    !supportInitialSendButton ||
    !supportModalCompose ||
    !supportReplyInput ||
  ...
  const buildPhotoPayload = async (file) => {
    ...
    return {
      imageBase64,
      imageMime: String(file.type || "image/jpeg").toLowerCase(),
    };
  };
  ...
  const clearPhotoSelection = () => {
    ...
    replyPhotoFile = null;
    supportInitialPhotoInput.value = "";
    supportReplyPhotoInput.value = "";
    supportInitialPhotoName.textContent = "Без файла";
  ...
  const buildMessageHtml = (item) => {
    ...
    const photoHtml = photoUrl
      ? `<img class="support-modal-message-photo" src="${escapeHtml(photoUrl)}" alt="Фото сообщения">`
      : "";
    return `
  ...
  const renderSupportState = (stickToBottom = false) => {
    ...
    supportModalMeta.textContent = hasChat
      ...
      : "Напишите вопрос, и мы ответим в этом чате.";
    supportModal.classList.toggle("is-compact", !hasChat);
    supportModalInitialForm.hidden = hasChat;
    supportModalCompose.hidden = !hasChat;
  ...
  const setLoadingState = (disabled) => {
    isLoading = disabled;
    supportInitialSendButton.disabled = disabled;
    supportReplySendButton.disabled = disabled;
    supportInitialQuestionInput.disabled = disabled;
    supportInitialUsernameInput.disabled = disabled;
    supportInitialPhotoInput.disabled = disabled;
  ...
  const sendInitialMessage = async () => {
    ...
    const questionText = String(supportInitialQuestionInput.value || "").trim();
    const usernameText = String(supportInitialUsernameInput.value || "").trim();
    if (!questionText && !(initialPhotoFile instanceof File)) {
      showAlert("Введите вопрос или добавьте фото");
      return;
    }

    setLoadingState(true);
    try {
      const imagePayload = await buildPhotoPayload(initialPhotoFile);
      ...
      supportInitialQuestionInput.value = "";
      clearPhotoSelection();
  ...
  supportModalExitButton.addEventListener("click", (event) => {
    event.preventDefault();
    closeSupportModal();
  });
  ...
  supportInitialPhotoInput.addEventListener("change", () => {
    initialPhotoFile = supportInitialPhotoInput.files?.[0] || null;
    supportInitialPhotoName.textContent = initialPhotoFile ? initialPhotoFile.name : "Без файла";
  });
  ...
  supportInitialSendButton.addEventListener("click", () => {
    sendInitialMessage().catch(() => {});
  });

  supportReplySendButton.addEventListener("click", () => {
    sendReplyMessage().catch(() => {});
  });

  supportInitialQuestionInput.addEventListener("keydown", (event) => {
  ...
  const renderSupportSection = async () => {
    adminSectionContent.innerHTML = `
      ...
      <div class="admin-support-layout">
        <section class="admin-support-chats">
        ...
        <section class="admin-support-pane" id="adminSupportPane" hidden>
          ...
          <div class="admin-support-compose">
            <textarea class="admin-textarea" id="adminSupportReplyInput" placeholder="Ответ пользователю"></textarea>
            <div class="admin-support-upload-row">
              <label class="admin-support-upload" for="adminSupportReplyPhotoInput">Фото</label>
    ...
    const renderChats = (items) => {
      ...
      chatsList.innerHTML = items
        .map((item) => {
          ...
          const title = String(item?.title || "").trim() || `Чат ${safeChatId}`;
    ...
    const renderMessages = (messages) => {
      ...
      messagesNode.innerHTML = messages
        .map((item) => {
          ...
          const photoUrl = resolvePhotoUrl(item?.photoUrl);
          const photoHtml = photoUrl
            ? `<img class="admin-support-message-photo" src="${escapeHtml(photoUrl)}" alt="Фото сообщения">`
            : "";
          return `
            <article class="${rowClass}">
    ...
    const openChat = async (chatId) => {
      ...
      try {
        ...
        const defaultTitle = String(chat?.title || "").trim();
    ...
    const sendReply = async () => {
      ...
      try {
        ...
        if (replyPhotoFile instanceof File) {
          const dataUrl = await fileToDataUrl(replyPhotoFile);
          imageBase64 = stripDataUrlPrefix(dataUrl);
          imageMime = String(replyPhotoFile.type || "image/jpeg").toLowerCase();
        }
