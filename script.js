...
(() => {
  ...
  const updateProfileOverview = () => {
    const localPurchases = historyItems.length;
  ...
  const renderHistory = () => {
    ...
    if (!filteredHistoryItems.length) {
      ...
      if (emptyTextNode) {
        const showNoResultsMessage = Boolean(historyItems.length) && (historyFilter !== "all" || historySearchQuery.trim());
        emptyTextNode.textContent = showNoResultsMessage
          ? (isEnglishLocale() ? "Nothing found. Try another filter." : "Ничего не найдено. Измените фильтр или поиск.")
          : (isEnglishLocale() ? "No purchases yet" : "Покупок пока нет");
      }
      if (profileHistoryEmptyAction) {
        const showAction = !historyItems.length;
        profileHistoryEmptyAction.hidden = !showAction;
      }
    ...
    visibleItems.forEach((item) => {
      ...
      subline.className = "profile-history-subline";
      ...
      subline.append(statusNode, date);

      const mainWrap = document.createElement("span");
      mainWrap.className = "profile-history-main-wrap";
      mainWrap.append(main, subline);

      const price = document.createElement("span");
      price.className = "profile-history-price";
      price.textContent = item.price || "—";

      row.append(iconWrap, mainWrap, price);
  ...
  const addCurrentOrderToHistory = (override = null) => {
    ...
    if (profileStatsFromApi && typeof profileStatsFromApi === "object") {
      ...
      profileStatsFromApi = {
        totalPurchases: Number.isFinite(currentPurchases) ? currentPurchases + 1 : currentPurchases,
        totalSpentRub:
          Number.isFinite(currentSpent) && Number.isFinite(entryPriceRub)
            ? Number((currentSpent + entryPriceRub).toFixed(2))
            : currentSpent,
    ...
    try {
      ...
      if (typeof window.starslixAddCurrentOrderToHistory === "function") {
        ...
        window.starslixAddCurrentOrderToHistory({
          ...
          status: promoErrorValue ? "warning" : "success",
        });
      }
