...
(() => {
  ...
  grids.forEach((grid) => {
    ...
    const syncPriceRow = (card) => {
      ...
      const sourceCard =
        card || cards.find((item) => item.classList.contains("is-active")) || cards[0] || null;
      if (!sourceCard) {
...
(() => {
  ...
  if (!historyItems.length) {
    historyItems = testPurchaseHistory.slice();
  ...
  if (typeof telegramUser?.photo_url === "string" && telegramUser.photo_url) {
    applyAvatarSrc(telegramUser.photo_url);
  } else {
    applyAvatarSrc("");
  }
...
(() => {
  ...
  const apiBase = (apiMeta?.content || "").trim().replace(/\/+$/, "");
  ...
  const syncPanelPriceRow = (gridNode) => {
    ...
    const sourceCard =
      gridNode.querySelector(".offer-card.is-active") || gridNode.querySelector(".offer-card");
    if (!sourceCard) {
      return;
    }
  ...
  const fetchMiniappConfig = async () => {
    try {
      ...
      console.warn("[miniapp] config load failed:", error);
    }
  };
  ...
  const submitOrder = async (button, panelView) => {
    ...
    // Основной режим для GitHub Pages / без отдельного backend:
    // отправляем данные прямо в бота через Telegram WebApp.sendData().
    if (window.Telegram?.WebApp?.sendData) {
      try {
        ...
        showMessage("Заявка передана боту. Если бот не отвечает, открой Mini App кнопкой из чата бота и повтори.");
        return;
      } catch (error) {
        console.error("[miniapp] sendData failed:", error);
      }
    }
    ...
    try {
      ...
      console.error("[miniapp] order submit failed:", error);
      showMessage(
