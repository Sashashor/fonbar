  ...
  const mergeHistoryCollections = (primaryItems, secondaryItems) => {
    ...
    const appendUnique = (list) => {
      (Array.isArray(list) ? list : []).forEach((item) => {
        ...
        const identity = `${normalized.type}|${normalized.main}|${normalized.price}|${normalized.createdAt}`;
  ...
  const renderPromosSection = async () => {
    adminSectionContent.innerHTML = `
      <div class="admin-form-grid">
        ...
        <label class="admin-form-field admin-form-field--full"><span class="admin-form-label">Действует до</span><input class="admin-input" id="adminPromoExpireInput" type="date"></label>
