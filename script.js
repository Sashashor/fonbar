...
(() => {
  ...
  const refreshButton = document.getElementById("reviewsRefreshButton");
  if (!leadersPage || !reviewsList || !refreshButton) {
    return;
  }
  ...
  const createAvatarFallback = (glyph) => {
    ...
    fallbackNode.className = "reviews-avatar-fallback";
  ...
  const renderEmptyState = (text) => {
    reviewsList.innerHTML = `<div class="reviews-empty">${escapeHtml(text)}</div>`;
  };
  ...
  const renderReviews = (items) => {
    ...
    reviewsList.innerHTML = items
      .map((item) => {
        ...
        const dateText = formatReviewDate(item?.createdAt);
        const avatarHtml = avatarUrl
          ? `<img class="reviews-avatar" src="${escapeHtml(avatarUrl)}" alt="${escapeHtml(authorName)}" data-fallback="${escapeHtml(fallbackGlyph)}">`
          : `<span class="reviews-avatar-fallback">${escapeHtml(fallbackGlyph)}</span>`;
          ...
          <article class="reviews-item">
            ...
            <div class="reviews-body">
              <div class="reviews-meta">
                <span class="reviews-author">${escapeHtml(authorName)}</span>
                <span class="reviews-date">${escapeHtml(dateText)}</span>
              </div>
              <p class="reviews-text">${escapeHtml(reviewText)}</p>
            </div>
          </article>
        `;
      })
    ...
    reviewsList.querySelectorAll(".reviews-avatar").forEach((avatarNode) => {
  ...
  const loadReviews = async ({ force = false } = {}) => {
    ...
    try {
      const response = await fetch(buildApiUrl("/api/miniapp/reviews?limit=40"), {
        method: "GET",
        cache: "no-store",
      });
  ...
  window.addEventListener("starslix:reviews-updated", () => {
    if (!leadersPage.classList.contains("is-active")) {
      return;
    }
    loadReviews({ force: true });
  });
  ...
  document.addEventListener("visibilitychange", () => {
    if (!document.hidden && leadersPage.classList.contains("is-active")) {
      loadReviews({ force: true });
    }
  });
})();
...
(() => {
  ...
  const mergeHistoryCollections = (primaryItems, secondaryItems) => {
    ...
    const appendUnique = (list) => {
      (Array.isArray(list) ? list : []).forEach((item) => {
        ...
        const identity = `${normalized.type}|${normalized.main}|${normalized.price}|${normalized.status}|${normalized.createdAt}`;
  ...
  const updateProfileOverview = () => {
    ...
    const levels = [0, 5, 15, 30, 60];
    const safePurchases = Math.max(0, Math.floor(Number(totalPurchases) || 0));
    let currentLevelIndex = 0;
    for (let index = 0; index < levels.length; index += 1) {
      if (safePurchases >= levels[index]) {
        currentLevelIndex = index;
      } else {
        break;
      }
    }
    const nextLevelTarget = levels[currentLevelIndex + 1] ?? null;
    const previousLevelTarget = levels[currentLevelIndex];
    ...
    let progressNote = isEnglishLocale() ? "Maximum level reached" : "Максимальный уровень достигнут";
    if (nextLevelTarget !== null) {
      const segment = Math.max(1, nextLevelTarget - previousLevelTarget);
      const filled = Math.max(0, Math.min(segment, safePurchases - previousLevelTarget));
      progressPercent = Math.round((filled / segment) * 100);
      const remaining = Math.max(0, nextLevelTarget - safePurchases);
      progressNote = isEnglishLocale()
        ? `To next level: ${remaining} purchases`
        : `До следующего уровня: ${remaining} покупок`;
    }
  ...
  const renderHistory = () => {
    ...
    visibleItems.forEach((item, index) => {
      ...
      row.append(iconWrap, main, statusNode, side);
...
(() => {
  ...
  const translations = {
    ru: {
      ...
      heroStars: "Stars",
      heroMobileLine3: "дешевле официальной цены",
      heroWideLine1: "Telegram Premium и",
      heroWideLine2Suffix: "дешевле официальной цены",
      ...
      dockContest: "Конкурс",
      dockProfile: "Профиль",
      leadersPageAria: "Страница отзывов",
      leadersTitle: "Отзывы",
      leadersText: "Отзывы из Telegram-группы",
      reviewsRefreshAria: "Обновить отзывы",
    ...
    en: {
      ...
      heroStars: "Stars",
      heroMobileLine3: "below official price",
      heroWideLine1: "Telegram Premium and",
      heroWideLine2Suffix: "below official price",
      ...
      dockContest: "Contest",
      dockProfile: "Profile",
      leadersPageAria: "Reviews page",
      leadersTitle: "Reviews",
      leadersText: "Reviews from the Telegram group",
      reviewsRefreshAria: "Refresh reviews",
  ...
  const applyLanguage = (lang) => {
    ...
    setAttr("#reviewsRefreshButton", "aria-label", dict.reviewsRefreshAria);
    setText("#reviewsEmpty", dict.reviewsLoading);
