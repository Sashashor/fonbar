...
(() => {
  ...
  paymentSelectNodes.forEach((node) => {
    ...
    node.querySelectorAll("[data-payment-method]").forEach((optionNode) => {
      optionNode.addEventListener("click", (event) => {
        ...
        if (nextMethod === "ton_wallet" && !boundTonWallet) {
          showMessage(
            isEnglishUi()
              ? "To pay via TON, tap Connect wallet under the price."
              : "Для оплаты через TON нажмите «Подключить свой кошелёк» под ценой."
          );
        }
        window.dispatchEvent(
  ...
  const renderTonWalletButtons = () => {
    const hasWallet = Boolean(boundTonWallet);
    const tonMethodActive = selectedPaymentMethod === "ton_wallet";
  ...
  const getOrderPayload = (panelView, options = {}) => {
    ...
    if (!previewOnly && paymentMethod === "usdt_ton") {
      return { error: "Метод USDT TON пока недоступен" };
    }
  ...
  const submitOrder = async (button, panelView) => {
    ...
    try {
      ...
      if (paymentUrl) {
        ...
        try {
          if (paymentProvider === "ton_wallet" && window.Telegram?.WebApp?.openLink) {
