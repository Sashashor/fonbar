...
(() => {
    ...
    node.querySelectorAll("[data-payment-method]").forEach((optionNode) => {
      optionNode.addEventListener("click", (event) => {
        ...
        if (nextMethod === "ton_wallet" && !boundTonWallet) {
          showMessage(
            isEnglishUi()
              ? "Link TON wallet in profile to pay via TON."
              : "Для оплаты через TON сначала привяжите кошелёк в профиле."
          );
        }
        window.dispatchEvent(
  ...
  const getOrderPayload = (panelView, options = {}) => {
    ...
    const tonPaymentSelected = paymentMethod === "ton_wallet";
    if (!previewOnly && paymentMethod === "usdt_ton") {
      return { error: "Метод USDT TON пока недоступен" };
    }
    if (!previewOnly && tonPaymentSelected && !boundTonWallet) {
    ...
    return {
      ...
      amount,
      promoCode,
      tonWallet: tonPaymentSelected ? boundTonWallet : "",
      paymentMethod,
      initData,
