...
(() => {
  ...
  const refreshButton = document.getElementById("homeReviewsRefreshButton");
  if (!homeReviews || !slider || !track || !dots || !emptyState || !refreshButton) {
    return;
  }
  ...
  const setSlide = (nextIndex, animate = true) => {
    ...
    currentSlide = Math.max(0, Math.min(slidesCount - 1, Number(nextIndex) || 0));
  ...
  const endDrag = (clientX) => {
    ...
    const deltaX = (Number(clientX) || 0) - dragStartX;
    isDragging = false;
    dragPointerId = null;
...
(() => {
  ...
  const applyLanguage = (lang) => {
    ...
    setAttr("#homeReviewsRefreshButton", "aria-label", dict.homeReviewsRefreshAria);
